...
class KimiService implements AIService {
  final String apiKey;
  late AIModel _currentModel;

  KimiService(this.apiKey) {
    _currentModel = availableModels.first;
  }

  @override
  String get providerName => 'Moonshot';

  @override
  String get brandAsset => 'assets/kimi_logo.png';

  @override
  List<AIModel> get availableModels => [
    AIModel(id: 'kimi-k2-5', displayName: 'Kimi K2.5', supportsVision: true, supportsWebSearch: true),
    AIModel(id: 'kimi-k2-thinking', displayName: 'Kimi K2 Thinking', supportsVision: false, supportsWebSearch: true),
  ];
  ...
  Future<String> sendMessage(String prompt, {required List<Map<String, dynamic>> history, File? attachment, bool isDeepResearch = false}) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return "Kimi Error: ${response.statusCode} - ${response.body}";
      }
    } catch (e) {
      return "Error connecting to Kimi: $e";
    }
  }

  @override
  Future<String> generateImage(String prompt) async {
    return "Kimi does not support image generation.";
  }
}